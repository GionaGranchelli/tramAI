...
internal fun parseWorkloadPrecondition(header: String?): WorkloadPrecondition {
    ...
    if (value.isNullOrEmpty()) return WorkloadPrecondition.Missing
    val digits =
        STRONG_NUMERIC_ETAG.matchEntire(value)?.groupValues?.get(1)
            ?: return WorkloadPrecondition.Unsupported
    val version = digits.toLongOrNull() ?: return WorkloadPrecondition.Unsupported
    return WorkloadPrecondition.Expected(WorkloadStateVersion(version))
}
