...
internal class WorkloadControlPlaneController(
    ...
    fun register(
        ...
        runBlocking {
            when (val outcome = commands.register(command.toIdentity(), command.toFingerprint(), command.toMetadata())) {
                is RegisterOutcome.Created -> {
                    created(outcome.registration)
                }
    ...
    fun updateMetadata(
        ...
        if (precondition !is WorkloadPrecondition.Expected) return preconditionFailure(precondition)
        return runBlocking {
            when (
                val outcome =
                ...
                is MetadataUpdateOutcome.Applied -> ok(outcome.registration)
                is MetadataUpdateOutcome.Unchanged -> ok(outcome.registration)
                is MetadataUpdateOutcome.Stale -> stale(outcome.currentVersion, outcome.expectedVersion)
                MetadataUpdateOutcome.NotFound -> notFound()
            }
        }
    }
        ...
        return runBlocking {
            when (
                ...
                is LifecycleTransitionOutcome.Stale -> {
                    stale(outcome.currentVersion, outcome.expectedVersion)
                }
    ...
    // ── Transport mapping (no authority logic) ──────────────────────

    private fun ok(read: ClassifiedRead): ResponseEntity<Any> =
        ResponseEntity
            .ok()
    ...
    private fun ok(registration: RegisteredWorkload): ResponseEntity<Any> =
        ResponseEntity
            .ok()
            .eTag(workloadEtag(registration.stateVersion))
            .body(WorkloadRegistrationResponse.from(registration))

    private fun created(registration: RegisteredWorkload): ResponseEntity<Any> =
        ResponseEntity
    ...
    /**
     * Precondition failed: the version the command was conditioned on is no longer current.
     * Distinct from a domain conflict; the body carries both versions and the response carries the
     * current ETag so a client can re-read and reconcile without parsing prose.
     */
    private fun stale(
    ...
    /** Missing precondition (428) or an unacceptable `If-Match` form (400). Never a silent write. */
    private fun preconditionFailure(precondition: WorkloadPrecondition): ResponseEntity<Any> =
        when (precondition) {
            ...
            WorkloadPrecondition.Unsupported -> {
                problem(
                    ...
                    title = "Unsupported If-Match precondition",
                    detail =
    ...
    private companion object {
        const val BASE_PATH = "/control-plane/workloads"
...
private fun WorkloadRegistrationCommand.toFingerprint(): ConfigurationFingerprint = ConfigurationFingerprint(configurationFingerprint)
