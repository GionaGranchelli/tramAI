...
class CompilerWarningsPlugin : Plugin<Project> {
    ...
    override fun apply(project: Project) {
        ...
        project.dependencies.add("kotlinCompiler", "org.jetbrains.kotlin:kotlin-compiler-embeddable:$KOTLIN_COMPILER_VERSION")

        val configDir = project.layout.projectDirectory.dir("config").dir("warnings")
        ...
        project.gradle.projectsEvaluated {
            ...
            project.logger.lifecycle("compiler-warnings: collected ${collected.size} compile units, ${compileTasks.size} compile tasks")
        }
    }
    ...
    private fun collectCompileUnits(project: Project): List<CompileUnitSpec> {
        val units = mutableListOf<CompileUnitSpec>()
        for (p in collectProjects(project)) {
            val sourceSets = p.extensions.findByType(SourceSetContainer::class.java) ?: continue
            for (ss in listOf("main", "test", "testFixtures")) {
                if (sourceSets.findByName(ss) == null) continue
                if (!File(p.projectDir, "src/$ss/kotlin").isDirectory) continue
                val compileTaskName =
                    when (ss) {
                        "main" -> "compileKotlin"
                ...
                val compileTask = p.tasks.findByName(compileTaskName) as? KotlinCompile ?: continue
                val compilerArgs =
                    try {
                        compileTask.compilerOptions.freeCompilerArgs.get()
                    } catch (e: Exception) {
                        emptyList()
                    }
                val jvmTarget =
                    try {
                        compileTask.compilerOptions.jvmTarget.orNull?.target ?: "21"
                    } catch (e: Exception) {
                        "21"
                    }
                units +=
