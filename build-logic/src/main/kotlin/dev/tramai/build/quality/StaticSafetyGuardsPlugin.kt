...
class StaticSafetyGuardsPlugin : Plugin<Project> {
    override fun apply(project: Project) {
        ...
        project.tasks.register<VerifyStaticSafetyGuardsTask>("verifyStaticSafetyGuards") {
            group = "verification"
            description =
                "Fail-closed production static safety guards for lifecycle, HTTP, logging, and forbidden APIs."
            configFile.set(project.layout.projectDirectory.file("config/quality/static-safety-guards.yml"))
            sourceFiles.from(sources)
            repositoryRoot.set(project.rootDir.absolutePath)
            reportsDir.set(project.layout.buildDirectory.dir("reports/static-safety-guards"))
        }
    }
}
