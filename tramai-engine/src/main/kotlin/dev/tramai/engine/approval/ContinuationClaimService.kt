...
internal class ContinuationClaimService(private val approvalContinuationStore: ApprovalContinuationStore?) {
    ...
    suspend fun claim(approvalId: String, expectedVersion: Long, claimedBy: String): ClaimedApprovalContinuation = requireStore().claimForExecution(approvalId, expectedVersion, claimedBy)
    suspend fun complete(approvalId: String, expectedVersion: Long, completedBy: String): ApprovalContinuation = requireStore().complete(approvalId, expectedVersion, completedBy)
    suspend fun cancel(approvalId: String, expectedVersion: Long): ApprovalContinuation = requireStore().cancel(approvalId, expectedVersion)
    private fun requireStore(): ApprovalContinuationStore =
