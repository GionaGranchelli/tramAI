...
class ToolRetryPolicyTest {
    ...
    @Test fun `non transient results never retry`() { listOf(ToolResult.Success("x"), ToolResult.InvalidInput("x"), ToolResult.PermanentFailure("x")).forEach { assertThat(policy.decide(it, testTool(idempotent = true), 0, 2)).isEqualTo(ToolRetryDecision.Stop(ToolFailureCode.EXECUTION_FAILED)) } }
