...
class ToolResultSanitizerTest {
    ...
    @Test fun `aggregate limit rejects and emits event`() { val events = mutableListOf<Pair<String, Map<String, Any?>>>(); val s = sanitizer(object : DlpInterceptor { override fun inspect(context: DlpContext, text: String) = DlpResult(text) }, limit = 5, events = events); assertThatThrownBy { runBlocking { s.sanitize(Message(MessageRole.TOOL, "abcdef")) } }.hasMessageContaining("exceeds aggregate input limit"); assertThat(events.single().first).isEqualTo(DLP_TOOL_REJECTED_METRIC); assertThat(events.single().second).containsEntry("reasonCode", "aggregate_text_limit_exceeded") }
