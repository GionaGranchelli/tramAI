...
class DefaultApprovalGatewayTest {
    ...
    fun setUp() {
        ...
        factory =
            FakeApprovalGatewayRequestFactory(
    ...
    @Test
    fun `new request persists approval, suspended invocation, and continuation`(): Unit =
        runBlocking {
    ...
    @Test
    fun `new request returns immediately without blocking`(): Unit =
        runBlocking {
            val gateway = createGateway()
    ...
    @Test
    fun `existing pending approval returns Suspended without duplicate create`(): Unit =
        runBlocking {
    ...
    @Test
    fun `existing approved approval returns AlreadyApproved`(): Unit =
        runBlocking {
    ...
    @Test
    fun `existing denied approval returns AlreadyDenied`(): Unit =
        runBlocking {
    ...
    @Test
    fun `existing timed-out approval returns Expired`(): Unit =
        runBlocking {
            ...
            val timeoutingContinuationStore =
                InMemoryApprovalContinuationStore(
                    clock = mutableClock,
                    maxContinuationTtl = Duration.ofHours(2),
                )
    ...
    @Test
    fun `expired-by-clock approval returns Expired`(): Unit =
        runBlocking {
    ...
    @Test
    fun `cancellation exception from store is rethrown`(): Unit =
        runBlocking {
            val failingStore = CancellingApprovalStore(approvalStore)
            val gateway =
                DefaultApprovalGateway(
    ...
    @Test
    fun `factory failure does not write stores`(): Unit =
        runBlocking {
            val failingFactory =
                object : ApprovalGatewayRequestFactory {
                    override suspend fun createRequest(
                        subject: ApprovalSubject,
                        recommendation: ApprovalRecommendation,
                        requiredRole: ApproverRole,
                        workflowRunId: WorkflowRunId?,
                    ): ApprovalGatewayPersistenceRequest = throw RuntimeException("factory-exploded")
                }
            val gateway =
                DefaultApprovalGateway(
                    approvalStore = approvalStore,
    ...
    fun `a caller run id disagreeing with the canonical identity is rejected before any write`(): Unit =
        runBlocking {
            ...
            val thrown =
                try {
                    withContext(GovernedRunScope(governedRun)) {
                        gateway.requestApproval(
                            subject = ApprovalSubject("claim-42"),
                            recommendation = ApprovalRecommendation("review", "Medical review required"),
                            requiredRole = ApproverRole("medical-reviewer"),
                            workflowRunId = WorkflowRunId("wf-run-1"),
                        )
                    }
                    null
            ...
            assertThat(thrown).isInstanceOf(GovernedRunContinuityException::class.java)
            assertThat(thrown!!.message).contains("governed-run-1")

            // Fail closed means nothing reaches durable state: no approval, no suspension, no
            // continuation, and no silent downgrade to a legacy un-attributed suspension.
    ...
    // -----------------------------------------------------------------------
    // 10. ResumeToken is the public credential, not the approval token digest
    // -----------------------------------------------------------------------

    @Test
    fun `suspended result uses public resume token from factory not approval token digest`(): Unit =
        runBlocking {
    ...
    @Test
    fun `existing pending result uses factory resume token not stored digest`(): Unit =
        runBlocking {
...
internal class FakeApprovalGatewayRequestFactory(
    ...
    override suspend fun createRequest(
        ...
        val suspendedInvocationMetadata =
...
/**
 * Delegating approval store whose `create` always cancels, used to prove the gateway rethrows
 * cancellation instead of swallowing it.
 */
private class CancellingApprovalStore(
