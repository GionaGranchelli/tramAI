...
import io.aurora.engine.create
...
class OpenTelemetryOperationObserverTest {
    ...
    fun `records parse failure event before successful retry`() {
        ...
        assertThat(firstAttempt.events).anySatisfy { event ->
            assertThat(event.name).isEqualTo("aurora.parse.failure")
            assertThat(event.attributes.asMap().keys.map { it.key }).contains("aurora.raw_response", "aurora.validation_error")
        }
        assertThat(firstAttempt.attributes.asMap())
            .containsEntry(io.opentelemetry.api.common.AttributeKey.booleanKey("aurora.structured.parse_success"), false)
    }
}
