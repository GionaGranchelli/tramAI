...
- **Unified Spring Boot starter (Spring track S6).** One canonical starter `dev.tramai:tramai-spring-boot-starter` now composes both runtime integrations: `tramai-spring-core` (standard) and the new `tramai-spring-sovereign` (sovereign). Runtime selection is explicit and profile-only — `tramai.profile=standard` (or missing) yields exactly one `Tramai`, `tramai.profile=sovereign` yields exactly one `SovereignTramai` — with the same `@AiService`/`@AiTool`/constructor-injection programming model under both. The former `tramai-spring-boot-starter-sovereign` artifact and its implicit sovereign-default `EnvironmentPostProcessor` are removed; sovereign mode must now be selected deliberately via `tramai.profile=sovereign`. The sovereign ops/persistence starters now build on `tramai-spring-sovereign`. Migration: replace the old coordinate with `tramai-spring-boot-starter` and configure `tramai.profile=sovereign`.
