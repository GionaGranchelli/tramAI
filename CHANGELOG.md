...
- Added Preview `ApprovedContinuationResumeWorker` for automatic resumption of approved continuations through the engine resume runtime (PR #112). Includes a `SovereignOpsApprovedContinuationResumeStore` for queue management, configuration support (`tramai.sovereign.ops.approved-resume-worker.enabled`), sealed credential decryption, and a background poll/schedule loop. The worker reads encrypted resume credentials from `tramai_approval_resume_credentials`, decrypts them using the sovereign store encryption key, and replays continuations through the `ApprovalResumeControlPlane`. Transient failures are retried with exponential backoff. Terminal failures move the record to a dead state.
- Added worker lifecycle, queue status store, queue snapshot, Micrometer metrics, and health indicator for the approved-resume worker (PR #113–#116). The `SovereignOpsApprovedContinuationResumeStore` provides queue status queries (`QUEUED`, `RUNNING`, `RETRYING`, `DEAD`). A queue snapshot endpoint returns aggregate counts per status. Micrometer counters and timers track resume attempts, successes, failures, and retries. A Spring Boot health indicator reports worker status as UP/DOWN/DEGRADED. All surfaces sanitize internal details — no resume tokens, credential digests, or workflow payloads are exposed.
