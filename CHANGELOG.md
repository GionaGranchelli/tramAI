...
- Added Spring auto-configuration for OpenAI-compatible local providers (PR #138). Creates a new `tramai-spring-boot-starter-local-provider-openai` module with `OpenAiCompatibleProviderAutoConfiguration` that maps `tramai.providers.*` YAML entries to `OpenAiCompatibleProvider` beans. The `SovereignLabProfileSmokeTest` now proves the `local-lab-provider` bean is created from YAML configuration rather than test-only manual registration.
