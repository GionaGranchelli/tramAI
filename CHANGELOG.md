...
|- Added Preview approval reviewer UI, disabled by default (PR #110). Serves a self-contained HTML page with inline CSS/JS at `/tramai/sovereign/reviewer` for listing, filtering, inspecting, approving, denying, and resuming approval work items through the existing REST control plane. Requires `reviewer-ui-enabled=true`, `rest-control-plane-enabled=true`, and `ApprovalInboxQueryService`, `ApprovalDecisionControlPlane`, `ApprovalResumeControlPlane` beans. Includes auto-configuration guards and MVC tests for status and sensitive-field safety.
