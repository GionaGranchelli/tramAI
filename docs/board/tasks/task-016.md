...
- bundled Vault and AWS Secrets Manager resolvers now ship through `tramai-spring`
- OpenTelemetry metrics now have OTLP HTTP smoke coverage against a collector-facing export path
- `examples/kotlin-native-smoke-example` compiles and runs a GraalVM native binary that instantiates a real Tramai `@AiService`

### 2. Security & Compliance
- [x] **PII Masking**: Add pluggable interceptors to redact sensitive data before it leaves the JVM.
- [x] **Secret Management**: First-class integration with cloud secret stores (AWS/Vault) for API keys.
...
- [x] Rate limiting works correctly in high-concurrency simulations.
- [x] Tramai-based applications can compile to GraalVM Native Images.
- [x] Token usage metrics are visible in standard OTel collectors.
- [x] Security hooks, secret sourcing, caching, and native-image support are covered by automated tests or smoke tests.
