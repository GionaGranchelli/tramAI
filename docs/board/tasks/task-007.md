...
Aurora's observability claim needs a concrete implementation path that works automatically when OTel is present and disappears cleanly when it is not.

## Scope

- add span wrapping around provider execution
- map Aurora metadata to GenAI semantic attributes
...
- observability can be added without forcing its dependencies into `aurora-standalone`
