...
Parsing without recovery is not enough. Tramai needs a robust correction loop and explicit failure reporting for structured outputs.
...
- consume structured failure results from `tramai-structured` without embedding parsing logic in the engine
