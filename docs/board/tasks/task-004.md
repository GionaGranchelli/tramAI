...
Structured output is Aurora's main product differentiator, so schema generation and parsing need to become concrete before provider work broadens.

## Scope

- implement custom Jackson-based schema generation
- map Kotlin nullability and Aurora annotations into schema output
- cache schema per operation
- implement raw text to JSON extraction and deserialization
- define the structured result contract returned to `aurora-engine`
...
Do not let this task pull retry policy into `aurora-structured`.
