...
Without a BOM, a consumer who mixes versions (e.g., `tramai-core:0.2.0` with `tramai-engine:0.3.0`) risks `NoSuchMethodError`, binary-incompatible SPI types, or broken annotation processing at runtime. The BOM eliminates this category of error entirely.
...
Gradle's `platform()` notation activates the version constraints from the BOM. All three modules resolve to `0.2.0` — the version declared in the BOM for each.
...
When published, Gradle resolves each `project()` reference to the **current project version** (`tramaiVersion`, default `0.2.0`). The resulting POM contains a `<dependencyManagement>` block that lists every module with its resolved version.
