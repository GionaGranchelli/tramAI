...
- PR #16 scope: continuation domain model, `ApprovalContinuationStore` SPI, `InMemoryApprovalContinuationStore`, `SensitiveToolArguments` wrapper, `Sha256ToolArgumentsDigester`, atomic claim-for-execution semantics, completion and expiry semantics, strict validation and leakage tests.
