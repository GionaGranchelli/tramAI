...
- PR #16 scope: continuation metadata/payload split, metadata-only `ApprovalContinuation`, `ApprovalContinuationStore` SPI including `sweepExpired()`, `ClaimedApprovalContinuation`, `InMemoryApprovalContinuationStore`, `SensitiveToolArguments` wrapper, `Sha256ToolArgumentsDigester`, approval-expiry binding via `approvalExpiresAt`, lazy expiry on touch paths, explicit sweep-based idle expiry cleanup, atomic one-time payload release and scrubbing semantics, completion/expiry/cancel metadata retention, strict validation, and leakage tests.
