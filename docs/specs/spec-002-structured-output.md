...
Tramai's main differentiation depends on turning typed return values into reliable structured outputs rather than raw text blobs. The library needs a consistent schema, parsing, validation, and retry pipeline for Kotlin data classes and Java POJOs.
...
- Retry policy ownership inside `tramai-structured`
- Streaming or partial structured responses

## Functional Requirements

- Tramai must generate JSON schema from supported return types using Kotlin nullability and Tramai annotations.
...
- `tramai-structured` must return a structured result object that represents success or failure and includes enough detail for the engine to decide whether to retry.
...
- Retry orchestration remains owned by `tramai-engine`, which consumes the structured result and decides whether to append feedback, retry, or fail.
...
- The boundary with `tramai-engine` must remain narrow and avoid exposing parsing internals as orchestration APIs.
...
- `tramai-structured` should report structured-output outcomes; `tramai-engine` should own what happens next.
