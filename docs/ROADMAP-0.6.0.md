...
- ✅ **Fail-closed architecture verifier (two layers):** ASM LDC bytecode scan of the four core modules' built jars (catches `const` inlining) plus a repository-wide source scan of every `tramai-*` module; declared Spring config-property namespaces are the only sanctioned non-catalogue literals, and the sovereign outbox subtree is explicitly excluded from that allowance. Mutation-verified on both layers (injected literal → red); the test task declares all modules' sources as inputs so the guard re-runs on any module change. The guard found and drove the migration of 40+ real un-catalogued literals across all modules.
