...
Tramai is intentionally structured so new providers and observers can be added without changing the public service model.
...
Implement `OperationObserver` from `tramai-core` and attach it through the standalone builder.
...
Check whether the feature belongs in the current shape of Tramai.
