...
> ⚠️ **Staleness notice.** This graph is generated from the `v0.5.0` baseline and is **not** kept in sync with the current module set automatically. It is a historical reference; the live authorities are distinct:
...
> - `verify060Architecture` — current topology/boundary/resolved-edge verification.
> Regeneration of this graph is owned by the build-architecture track — do not edit it by hand to chase the live module set.
