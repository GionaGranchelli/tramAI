...
> **Build coordinates:** `dev.tramai:<module>:0.6.0`
...
dependencies {
    // Always needed
    implementation("dev.tramai:tramai-core:0.6.0")
    implementation("dev.tramai:tramai-engine:0.6.0")

    // Pick ONE provider
    implementation("dev.tramai:tramai-ollama:0.6.0")   // local models
    // implementation("dev.tramai:tramai-openai:0.6.0") // OpenAI / compatible APIs
    // implementation("dev.tramai:tramai-anthropic:0.6.0") // Claude

    // Optional: structured output
    implementation("dev.tramai:tramai-structured:0.6.0")

    // Pick adapter
    // implementation("dev.tramai:tramai-standalone:0.6.0")     // No framework
    // implementation("dev.tramai:tramai-spring-boot-starter:0.6.0") // Spring Boot
...
|--------|-------|---------|-----------|----------|-------------|----------------|
| `tramai-core` | Core | Annotations (`@AiService`, etc.), Models (`Message`, `ContentPart`), SPIs (`ModelProvider`, `Capability`), UsageMetrics. | none | `tramai-core:0.6.0` | **Always.** Every TramAI project needs this. | Never. |
| `tramai-engine` | Core | Proxy generation, Method dispatch, Capability validation, Retry, Tool registries. | core | `tramai-engine:0.6.0` | **Always** in production. | Never. |
| `tramai-structured` | Core | JSON Schema generation from Kotlin types → validate → retry on failure. | core | `tramai-structured:0.6.0` | Method returns `data class`, `enum`, `List<T>`, etc. | Only returning `String`. |
| `tramai-memory` | Core | Multi-turn chat persistence: `TokenAwareChatMemory`, `PersistentChatMemory`. | core | `tramai-memory:0.6.0` | Conversational agents, multi-turn contexts. | Stateless endpoints. |
| `tramai-memory-store` | Core | SPI for external persistence of conversation history. | core | `tramai-memory-store:0.6.0` | When `ChatMemory` must outlive JVM restarts. | In-memory use cases. |
| `tramai-rag` | Core | Pipeline for ingestion, chunking, retrieval, and RAG context injection. | core | `tramai-rag:0.6.0` | Document Q&A or internal knowledge base flows. | No document loading. |
| `tramai-embedding`| Core | Core SPI for text embedding models. | core | `tramai-embedding:0.6.0` | When using RAG or `vectorstore` modules. | No RAG usage. |
| `tramai-vectorstore-spi`| Core | Interfaces for storing and querying text embeddings and metadata. | embedding | `tramai-vectorstore-spi:0.6.0` | Any semantic search feature. | No RAG usage. |
| `tramai-vectorstore-chroma`| Impl | ChromaDB implementation of the vector store SPI. | vectorstore-spi | `tramai-vectorstore-chroma:0.6.0` | You want to run Chroma DB locally or remotely. | Postgres/No RAG. |
| `tramai-vectorstore-pgvector`| Impl | PostgreSQL pgvector implementation of the vector store SPI. | vectorstore-spi | `tramai-vectorstore-pgvector:0.6.0` | You are using Postgres for your application data. | Chroma/No RAG. |
| `tramai-observability` | Tooling| OpenTelemetry spans, Worker Events (`onLeaseRenewed`, etc.). | core, orchestration | `tramai-observability:0.6.0` | Distributed tracing, metrics, audit trails. | No OTEL infrastructure. |
| `tramai-testing` | Tooling | Mock providers, Assertion helpers, Failure simulation, Request recording. | core | `tramai-testing:0.6.0` | **Always in tests.** | Never in production. |
| `tramai-bom` | Tooling | Version alignment BOM. Zero code. | none | `tramai-bom:0.6.0` | Multi-module projects. | Single-module project. |
...
| Module | Layer | Purpose | Depends on | Artifact | When to use | When NOT to use |
|--------|-------|---------|-----------|----------|-------------|----------------|
| `tramai-orchestration` | Orchestration | Multi-step workflows (`aiStep`, `parallelStep`, etc.). Distributed Worker Pool (leases, heartbeat, graceful shutdown). | core | `tramai-orchestration:0.6.0` | Chained AI calls, conditional branching, human-in-loop, distributed execution. | Simple request-response. |
