...
Tramai is now in the `0.5.x` line: credible as a library surface, broader in runtime scope, and still pre-`1.0`.
...
- bundled Vault and AWS Secrets Manager resolvers ship in `tramai-spring-boot-starter` (auto-config in `tramai-spring-core`); standalone usage still resolves secrets explicitly before provider construction
