...
- **Mechanism:** per-module declared-dependency vs source-reference analysis (import evidence + full-class/package evidence scanned from the dependency jars on the compile+runtime classpath union), main vs test configuration scoping. Imports are captured as full symbols (plain, wildcard `foo.bar.*`, Java static, Kotlin alias, trailing `;`) and matched against the exact top-level class names and packages found in the declared coordinate's jars — sibling artifacts sharing a package family (org.springframework.context vs org.springframework.jdbc) cannot justify each other. Not "Dependency Analysis Plugin Lite" — deliberately narrow.
