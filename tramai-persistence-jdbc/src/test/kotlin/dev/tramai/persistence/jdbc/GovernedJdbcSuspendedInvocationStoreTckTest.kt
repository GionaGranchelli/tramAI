...
/**
 * 0.7.1d: GovernedJdbcSuspendedInvocationStore must satisfy the same
 * SuspendedInvocationStore compatibility contract as the store it composes.
 *
 * The governed capability delegates into the JDBC store's single encrypted row,
 * so the released contract has to hold through the wrapper: same datasource,
 * same schema, same codec, per-case isolation owned here.
 */
...
class GovernedJdbcSuspendedInvocationStoreTckTest : SuspendedInvocationStoreTck() {
    ...
    fun setUpAll() {
        postgres =
            PostgreSQLContainer("postgres:17-alpine")
                .withDatabaseName("governed_suspended_tck")
                .withUsername("test")
                .withPassword("test")
        postgres.start()
        dataSource =
        ...
        val v1Sql =
            this::class.java.classLoader
                ...
                ?.readText()
                ?: error("V1 schema SQL resource not found")
    ...
    override fun createStore(): SuspendedInvocationStore {
        ...
        return GovernedJdbcSuspendedInvocationStore(
            JdbcSuspendedInvocationStore(dataSource, testCodec, clock = fixedClock),
        )
    }
}
