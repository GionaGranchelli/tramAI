...
class JdbcWorkloadRegistrationStaleAcrossInstancesTest {
    ...
    private fun newInstance(): WorkloadRegistrationAuthority = WorkloadRegistrationAuthority(JdbcWorkloadRegistrationStore(dataSource))
    ...
    fun `a stale expected version is rejected across instances and after reconstruction`() =
        runBlocking<Unit> {
            ...
            instanceA.register(scope, WorkloadRegistrationFixtures.fingerprint(), WorkloadRegistrationFixtures.metadata())
    ...
    fun `re-reading the authoritative version lets the stale writer reconcile and win exactly once`() =
        runBlocking<Unit> {
            ...
            val observed = writer.authoritative(scope.workloadId, scope.environmentId, scope.deploymentId)!!.observedVersion
