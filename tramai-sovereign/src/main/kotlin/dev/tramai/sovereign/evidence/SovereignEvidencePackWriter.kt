...
object SovereignEvidencePackWriter {
    ...
    private fun serialize(pack: SovereignEvidencePackV1): String {
        ...
        appendField(sb, "schemaVersion", pack.schemaVersion.toString(), 1, 11)
        appendStringField(sb, "deploymentMode", pack.deploymentMode, 2, 11)
        appendStringListField(sb, "allowedModels", pack.allowedModels, 3, 11)
        appendStringListField(sb, "allowedProviders", pack.allowedProviders, 4, 11)
        appendStringMapField(sb, "providerZones", pack.providerZones, 5, 11)
        appendObjectField(sb, "artifactVerificationSettings", pack.artifactVerificationSettings, 6, 11)
        appendObjectListField(sb, "verifiedModels", pack.verifiedModels, 7, 11, serialize = ::serializeVerifiedModel)
        appendObjectListField(sb, "artifacts", pack.artifacts, 8, 11, serialize = ::serializeArtifact)

        if (pack.zeroEgress != null) {
            appendObjectField(
                sb = sb, key = "zeroEgress", value = pack.zeroEgress,
                index = 9, total = 11, last = false,
                serialize = { z -> serializeZeroEgress(z) },
            )
        } else {
            appendNullField(sb, "zeroEgress", 9, 11, last = false)
        }

        if (pack.auditChain != null) {
            appendObjectField(
                sb = sb, key = "auditChain", value = pack.auditChain,
                index = 10, total = 11, last = false,
                serialize = { a -> serializeAuditChain(a) },
            )
        } else {
            appendNullField(sb, "auditChain", 10, 11, last = false)
        }

        appendStringField(sb, "generatedAt", pack.generatedAt, 11, 11, last = true)
    ...
    private fun serializeVerifiedModel(m: VerifiedModelEvidenceV1): String {
        val sb = StringBuilder()
        sb.appendLine("{")
        appendField(sb, "modelName", esc(m.modelName), 1, 4, indent = 2)
        appendStringField(sb, "providerId", m.providerId, 2, 4, indent = 2)
        appendStringField(sb, "revision", m.revision, 3, 4, indent = 2)
        appendField(sb, "artifactDigestRegistered", m.artifactDigestRegistered.toString(), 4, 4, indent = 2, last = true)
        sb.append("            }")
        return sb.toString()
    }
