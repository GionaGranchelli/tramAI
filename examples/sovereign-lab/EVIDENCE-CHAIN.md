...
| export runtime records | Policy, approval, and routing exporters | Produces `RuntimeEvidenceRecord` objects from runtime decisions |
| write runtime-evidence | `RuntimeEvidenceBundleWriter` (tramai-security) | Writes JSONL files into `runtime-evidence/` section |
| fill | Manual operator step | Captures command output and evidence |
| finalize | `finalize-evidence-bundle.sh` | Refreshes manifest digests after all evidence is in place |
| verify | `verify-evidence-bundle.sh` | Checks manifest, files, digests, sizes, paths, claim boundary, and runtime-evidence structural integrity |
...
Phase 5 produced the three runtime evidence exporters (#184–#186), the [Runtime Evidence Bundle Map](../../docs/evidence/runtime-evidence-bundle-map.md), and the `RuntimeEvidenceBundleWriter` (#199) that wires them into the sovereign evidence bundle. Policy, approval, and provider-routing evidence records are now written into the `runtime-evidence/` section as JSONL files by explicit `RuntimeEvidenceBundleWriter` invocation. The verifier checks structural integrity of every JSONL line, enforces metadata allowlists, and rejects privacy violations. Runtime evidence is not generated automatically or by the bundle creation scripts — explicit writer invocation is required.

The evidence bundle `runtime-evidence/` directory contains:
