...
Phase 5 produced the three runtime evidence exporters (#184–#186) and the [Runtime Evidence Bundle Map](../../docs/evidence/runtime-evidence-bundle-map.md) that defines where policy, approval, and provider-routing evidence records belong in the sovereign evidence bundle. Runtime evidence is not yet generated by the bundle creation scripts — the exporters exist and the bundle layout is documented, but no `runtime-evidence/` files are committed to the lab directory at this stage.
