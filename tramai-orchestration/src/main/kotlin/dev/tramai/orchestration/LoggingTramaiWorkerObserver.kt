...
class LoggingTramaiWorkerObserver(
    ...
    override fun onLeaseRenewalFailed(workflowId: String, workerId: String, error: Throwable) = logEvent("workflow=$workflowId worker=$workerId event=lease_renewal_failed error=${error.message}")
    override fun onLeaseReleaseFailed(workflowId: String, workerId: String, error: Throwable) = logEvent("workflow=$workflowId worker=$workerId event=lease_release_failed error=${error.message}")
    override fun onPollFailed(workerId: String, error: Throwable) = logEvent("worker=$workerId event=poll_failed error=${error.message}")
    ...
    override fun onStepAttemptFailed(runId: String, stepName: String, attemptId: String, workerId: String, error: Throwable) = logEvent("run=$runId step=$stepName attempt=$attemptId worker=$workerId event=attempt_failed error=${error.message}")
